...
for split in ['train', 'val']:
    ...
    for item in tqdm(anno_data):
        ...
        prompt = f"According to the given description, \"{situation},\" please provide the location of yourself that closely matches this description."
        prompt += f" {question} Answer the question using a single word or phrase."

        pos = np.array([
            item['position']['x'], item['position']['y'], item['position']['z'], item['rotation']['_x'],
            item['rotation']['_y'], item['rotation']['_z'], item['rotation']['_w']
        ]).reshape(1, 7)
        ...
        # print((dist < 0.5).sum() / len(dist), min_dist)
        ...
        if split == "train":
            ...
            if answer[-1] != ".":
                answer += "."
            caption = "My location is <GRD>. Answer: " + answer
    ...
    print(f"dist > 1.0: {count / len(new_annos)}")
